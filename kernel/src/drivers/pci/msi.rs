...
///
/// Returns `Some(())` on success, `None` if the device lacks an MSI capability.
pub fn program(dev: &PCIFunction, vector: HardwareInterrupts) -> Option<()> {
    let cap = u16::from(dev.find_capability(0x05)?);
    ...
    let msg_addr = 0xFEE0_0000 | (lapic_id << 12);
